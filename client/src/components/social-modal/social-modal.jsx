...
const SocialModal = ({artwork,artworkPageUrl,toggleModal, showModal }) => {
  ...
  return (
   ...
   { showModal && (    <div className='social-modal'>
        <HelmetMetaData title={title} currentUrl={artworkPageUrl} imageUrl={url}></HelmetMetaData>
        <GrFormClose className="close-social-modal-button" aria-label='close modal' onClick={()=>{toggleModal()}}/>
        <SocialFollow 
        ...
        />
         <div className='copy-link-container'>
                 <input  className="copy-input" type="text" defaultValue={`${artworkPageUrl}`} value={artworkPageUrl} readOnly id="myInput"/>
                <button className="copy-button" onClick={copy}><AiOutlineCopy/></button>
        </div>
      </div>)}
   </div>
  )
